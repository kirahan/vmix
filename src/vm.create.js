...
module.exports.create=function(vm,node,scope,pel,insert_pos){
	...
	var newvmopt=core_vm.createcommon.calcommon(vm,node,scope);
	if(node.dir['el-hook'] && node.dir['el-hook'].indexOf('node:')==0){
		core_vm.elhook(vm,null,node.dir['el-hook'].substr(5),'beforeCreated',node);
	}
	...
	if(newvmopt){
		...
		if(node.dir['el-hook'])core_vm.elhook(vm,thisel,node.dir['el-hook'],'selfCreated');
		if(node.childNodes && node.childNodes.length>0 && !node.childskip_inject){
			core_vm.create.nodes(vm,node,scope,thisel);
		}
		if(node.dir['el-hook'])core_vm.elhook(vm,thisel,node.dir['el-hook'],'childrenCreated');
	}
	return thisel;
}
module.exports.nodes=function(vm,node,scope,pel,insert_pos,iflist){
	...
	for(var sn=0,len=nodes.length;sn<len;sn++){
		if(nodes[sn].dir && nodes[sn].dir['el-filter']){
			needif=1;
			break;
		}
	}
	for(var sn=0,len=nodes.length;sn<len;sn++){
		...
		if(node.$injectscope){
			if(node.$injectfromvm_sid){
				var inject_vm=core_vm.gcache.vmsbysid[node.$injectfromvm_sid];
				if(!inject_vm){
					core_vm.devalert('no inject_vm')
					continue;
				}else{
					vm=inject_vm;
				}
		...
		if(needif){
			if(!node.dir || !node.dir['el-filter']){
				if_result[sn]=true;
				start_index=sn;
			}else{
				core_vm.calif.single(vm,if_result,node.dir['el-filter'],sn,scope,start_index);
				if(node.dir['el-filter']=='else')start_index=sn+1;
			}
		}
